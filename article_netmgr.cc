...
qint64 ArticleResourceReply::readData( char * out, qint64 maxSize )
{
  ...
  GD_DPRINTF( "====reading already readed: %d of %d bytes, toRead:%d.  Finished: %d", alreadyRead, avail, (int)toRead, finished );
...
void LocalSchemeHandler::requestStarted(QWebEngineUrlRequestJob *requestJob)
{
  ...
  connect( reply, &QNetworkReply::readyRead, requestJob, [ = ]() {
