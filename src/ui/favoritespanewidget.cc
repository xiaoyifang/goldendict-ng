  ...
  if ( m_wal->hasEntries() ) {
    ...
    for ( const auto & op : operations ) {
      try {
        ...
        qWarning() << "Exception occurred while replaying WAL entry, skipping:" << op.toString();
        continue;
      }
    }
...
QModelIndex FavoritesModel::addNewFolder( const QModelIndex & idx )
{
  ...
  if ( m_wal ) {
    ...
    m_wal->logAdd( fullPath, true ); // 文件夹操作
  }
...
bool FavoritesModel::addNewWordFullPath( const QString & headword )
{
  ...
  if ( m_wal ) {
    m_wal->logAdd( fullPath, false ); // 单词操作
  }
