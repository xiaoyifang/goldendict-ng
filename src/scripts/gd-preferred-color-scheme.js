  ...
  function gdNeutralizeDarkMedia(){
    try {
      ...
      for(var si=0; si<sheets.length; si++){
        var rules;
        try { rules = sheets[si].cssRules || sheets[si].rules; } catch(e){ continue; } // cross-origin
        if(!rules) continue;
        for(var i=rules.length-1; i>=0; i--){
          ...
          if(r && r.type === 4 /* CSSRule.MEDIA_RULE */){
            ...
            if(cond.indexOf('prefers-color-scheme') !== -1 && cond.indexOf('dark') !== -1){
              try { sheets[si].deleteRule(i); } catch(e){}
            }
          }
        }
      }
    ...
    new MutationObserver(function(ms){
      for(var mi=0; mi<ms.length; mi++){
        ...
        for(var ni=0; ni<ns.length; ni++){
          ...
          if(n.tagName === 'LINK' && (n.rel||'').indexOf('stylesheet') !== -1) gdScheduleSheet(n);
